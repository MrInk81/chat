...
// ----- PWA install (banner-aware)
let deferredPrompt=null;
window.addEventListener('beforeinstallprompt',(e)=>{
  e.preventDefault();
  deferredPrompt=e;
  if(isPhone && installBanner){
    installBanner.style.display='flex';
  }else{
    installBtn.style.display='inline-flex';
  }
  fixComposerScrollPadding();
});
...
// Sidebar auto-close on small screens and fix padding
  const sidebar = document.querySelector('.side');
  if (window.matchMedia('(max-width:1024px)').matches && sidebar) {
    sidebar.classList.remove('open');
  }
  try { fixComposerScrollPadding(); } catch(e){}
});
...
// ----- Layout helpers (mobile safe padding etc.)
function fixComposerScrollPadding(){
  const scroller = document.querySelector('.messages');
  if(!scroller || !composerEl) return;
  const composerH = composerEl.getBoundingClientRect().height || 0;
  const bannerEl = installBanner;
  const bannerVisible = bannerEl && getComputedStyle(bannerEl).display!=='none';
  const bannerH = bannerVisible ? (bannerEl.getBoundingClientRect().height||0) : 0;
  const bottomSafe = composerH + bannerH + 8;
  scroller.style.paddingBottom = bottomSafe + 'px';
}
s(inboxCache).filter(it=>it && it.type!=='warn').length;
...
/* ----- Mobile quick room switch helper (exposed globally) ----- */
window.activateRoom = function(rid){
  try{
    currentRoom = rid;
    renderRooms();
    loadMessages();
...
// ----- Bootstrap
document.addEventListener('DOMContentLoaded', ()=>{ renderRooms(); loadMessages(); try{applyMuteBlur();}catch(e){} fixComposerScrollPadding(); window.addEventListener('resize', fixComposerScrollPadding); if(!auth.currentUser){ openWelcome(); } });
