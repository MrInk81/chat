...
function applyMuteBlur(){
  try{
    ...
    document.querySelectorAll('.messages .text').forEach(el=>{
      if(muted){ el.classList.add('blur'); }
...
if(!auth.currentUser){
  const words=(m.text||'').split(/\s+/); const first=words.slice(0,3).join(' '); const rest=words.slice(3).join(' ');
  text.classList.add('blur'); text.innerHTML=`<span>${first}</span> <span class="rest">${rest}</span>`;
